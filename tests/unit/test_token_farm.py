...
# Testing owner and non-owner ability to call the function
def test_set_price_feed_contract():
    ...
    # When using get_contract for an address parameter, brownie will know to use the address and not the actual contract
...
def test_stake_tokens(amount_staked):
    ...
    # Assert
    assert (
        # Checking that the staking balance of dapp_token for our account = the amount we staked above
        token_farm.stakingBalance(dapp_token.address, account.address)
        == amount_staked
    )
    assert token_farm.uniqueTokensStaked(account.address) == 1
    assert token_farm.stakers(0) == account.address
    # return so we can use this test in other tests
...
def test_issue_tokens(amount_staked):
    ...
    if network.show_active() not in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        pytest.skip("Only for local testing!")
    account = get_account()
    # Grabbed from above test
    token_farm, dapp_token = test_stake_tokens(amount_staked)
    starting_balance = dapp_token.balanceOf(account.address)
    # Act
    token_farm.issueTokens({"from": account})
    # Assert
    # We are staking 1 dapp+token == in price to 1 ETH, so we should get 2,000 dapp tokens in reward. Since the price of eth in helpful_scripts INITIAL_PRICE_FEED_VALUE = $2,000
    assert (
